...
TUNNEL_SEED=$(echo -n "${SEED}rhophome" | sha256sum | cut -c1-24)
...
cat >/usr/local/etc/xray/config.json <<-EOF
	{
	  ...
	  "reverse": {
	    "portals": [
	      { "tag": "portal", "domain": "reverse.proxy" }
	    ]
	  },
	  "inbounds": [
	    {
	  ...
	  "routing": {
	    ...
	    "rules": [
	      ...
	      { "type": "field", "inboundTag": ["interconn"], "domain": ["full:reverse.proxy"], "outboundTag": "portal" },
...
if [[ "$OS_FAMILY" == "rhel" ]] && systemctl is-active --quiet firewalld 2>/dev/null; then
	...
	warning003="firewalld 已放行 80/tcp ${PORT}/tcp ${TUNNEL_PORT}/tcp(tunnel)"
fi
...
echo -n "$insert bash <(curl -fsSL ${BASEURL}rhop-cloud.sh) " >>~/_xray_url_
if [[ ${#args[@]} -gt 0 ]]; then
	echo -n "${args[*]}" >>~/_xray_url_
fi
echo "" >>~/_xray_url_
echo "" >>~/_xray_url_
echo "===== 家宽落地端安装 =====" >>~/_xray_url_
echo "TUNNEL_SEED=$TUNNEL_SEED HOST=$IPV4 bash <(curl -fsSL ${BASEURL}rhop-home.sh) $REGION_ID" >>~/_xray_url_
echo "------------------------------------" >>~/_xray_url_
